...
static int zforce_activate(struct zforce *zf)
{
	...
	ret = zforce_command(zf, packet, sizeof(packet), &packet[0]);	
	if (ret != 0)
		dev_warn(zf->dev, "zforce_activate: simple command failed\n");
...
static void zforce_restart(struct zforce *zf)
{
	...
	//flush_workqueue(zf->zforce_tasks);
	//cancel_work_sync(&zf->zforce_work);
...
out: 
	if(ret != 0)
		dev_info(zf->dev, "Zforce failed to restart with error %d \n", ret);
}
...
static void parse_data_task(struct work_struct *work)
{
	...
	unsigned long flags;
	u8 i, j, count, len, boot_status;
	enum zforce_cmd cmd;
	int ret, error_flag, boot_flag;
	error_flag = boot_flag = 0;
	...
	#ifdef ZFORCE_DEBUG
	//dev_info(zfp->dev, "zforce_parse: error = %d boot = %d\n", error_flag, boot_flag);
	#endif
	...
	len = zfp->packet_len;
	zfp->wq_count++;
	dev_info(zfp->dev, "zforce_parse: %d \n", zfp->wq_count);
	
	for(i = 0; i < ZFORCE_READ_RETRIES; i++)
	{
		ret = zfp->ops->read(zfp->dev, data, len);
		if(ret == 0) break;
		//else dev_info(zfp->dev, "zforce_parse: %d\n", i);
	}
	
	if(unlikely(0 != ret))
	{
		dev_warn(zfp->dev, "Failed read in parse data\n");
		error_flag = 1;
		goto out;
	}

	//ret = parse_data(zfp, data, recv[1]);
	//if(unlikely(0 != ret))
		//goto out;	
	...
	//dev_info(zf->dev, "parse_data: Command Parsed 0x%hhX \n", cmd);
		...
		//case FORCECALIBRATION:
		case SETACTIVELEDS:
			*zfp->cmd_data[cmd_to_idx[cmd]] = data[++i];
		...
		case BOOTCOMPLETE:
			...
			if(boot_status & 1)
			{
				dev_info(zfp->dev, "parse_data: Reboot failed.\n");
			}
			else
			{
				dev_info(zfp->dev, "parse_data: Reboot successful.\n");
				zfp->cmd_recv[cmd_to_idx[cmd]] = RECEIVED;
			}
			if(zfp->reboot_count > 1)
			{
				boot_flag = 1;
				goto out;
			}
...
out:
	if(error_flag) 
		dev_info(zfp->dev, "parse_data_task ERROR! \n");
	spin_unlock_irqrestore(&zfp->lock, flags);
	wake_up_interruptible(&zfp->wait);
	//if(boot_flag)
		//zforce_restart(zfp);
	//enable_irq(zfp->irq);
}
...
static irqreturn_t zforce_isr(int irq, void *priv)
{
	int ret;
	u8 recv[2];
	//struct zforce *zf = (struct zforce *)priv;
	zfp->isr_count++;
	dev_info(zfp->dev, "zforce_isr: %d \n", zfp->isr_count);
	
	ret = zfp->ops->read(zfp->dev, recv, sizeof(recv));
	if(unlikely(0 != ret))
	{
		dev_warn(zfp->dev, "Failed read in ISR \n");
		return IRQ_NONE;
	}

	if(unlikely(ZFORCE_FRAME_START != recv[0]))
	{
		dev_warn(zfp->dev, "Bad packet: %#hhx\n", recv[0]);
		return IRQ_NONE;
	}
	else
	{
	...
	zfp->packet_len = recv[1];
	
	queue_work(zfp->zforce_tasks, &zfp->zforce_work);
	//dev_info(zfp->dev, "Next packet = %d bytes \n", zfp->packet_len);
	return IRQ_HANDLED;
}
...
#ifdef ZFORCE_DEBUG
	dev_info(zf->dev, "zforce_init: %d %d\n", width, height);
#endif
...
static int zforce_init_ops(struct zforce *zf)
{
	...
	frequency.finger_freq = 100;
	frequency.stylus_freq = 100;
...
static void zforce_reset(u32 value)
{
	if(value)
	{
		// Reset Neonode in HW
...
static ssize_t set_start_restart(struct device *dev, struct device_attribute *attr, const char* buf, size_t count)
{
	u32 value;
...
static DEVICE_ATTR(restart, S_IWUSR|S_IWGRP|S_IWOTH, NULL, set_start_restart);
...
//	input_dev->evbit[0] = BIT(EV_KEY) | BIT(EV_ABS);
...
#ifdef ZFORCE_DEBUG
	dev_info(zf->dev, "width %u height %u\n", zf->settings.res.width, zf->settings.res.height);
	...
	dev_info(zf->dev, "major %u revision %u\n", zf->settings.fw_major, zf->settings.fw_revision);
...
static int zforce_i2c_read(const struct device *dev, u8 *val, unsigned int count)
{
	...
	dev_info(zfp->dev, "count = %d tmp = %d \n", count, tmp);
