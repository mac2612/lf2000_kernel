...
{
	u16 tmp = readw(pcdev->base + VIP_CLIP_YUYVENB);
	...
	writew(tmp, pcdev->base + VIP_CLIP_YUYVENB);
}
				...
				const u32 stride)
{
	...
	tmp = base >> 16;
	writew(tmp, pcdev->base + VIP_CLIP_BASEADDRH);

	tmp = base & 0xFFFF;
	writew(tmp, pcdev->base + VIP_CLIP_BASEADDRL);

	tmp = stride >> 16;
	writew(tmp, pcdev->base + VIP_CLIP_STRIDEH);

	tmp = stride & 0xFFFF;
	writew(tmp, pcdev->base + VIP_CLIP_STRIDEL);
}
...
static void set_decimator(struct lf2000_vip_dev *pcdev, u16 width, u16 height)
{
	...
	writew(width, pcdev->base + VIP_DECI_TARGETW);
	writew(height, pcdev->base + VIP_DECI_TARGETH);

	tmp = src_w - width;
	writew(tmp, pcdev->base + VIP_DECI_DELTAW);
	tmp = src_h - height;
	writew(tmp, pcdev->base + VIP_DECI_DELTAH);

	/* 2's complement registers */
	tmp = (s16)((width << 1) - src_w);
	writew(tmp, pcdev->base + VIP_DECI_CLEARW);
	tmp = (s16)((height << 1) - src_h);
	writew(tmp, pcdev->base + VIP_DECI_CLEARH);
}
		...
		const enum lf2000_vip_pipe pipe, const bool enable)
{
	...
	config	= readw(pcdev->base + VIP_CONFIG);
	intc	= readw(pcdev->base + VIP_INTCTRL) & (0x1 << VSINTENB);
	...
	else
	{
		...
		if((pcdev->capture == false) && (pcdev->overlay == false))
		{
			...
			odint	= readw(pcdev->base + VIP_ODINT);
			...
			writew(odint, pcdev->base + VIP_ODINT);
		}

		pcdev->cd_enb = enb;
		writew(pcdev->cd_enb, pcdev->base + VIP_CDENB);

		pcdev->output_pipe[pipe_idx] = LF2000_VIP_PIPE_IDLE;
	}

	writew(config, pcdev->base + VIP_CONFIG);
	writew(intc, pcdev->base + VIP_INTCTRL);
...
static int lf2000_camera_add_device(struct soc_camera_device *icd)
{
	...
	tmp = readw(pcdev->base + VIP_SYNCCTRL);
	tmp |= (0x3 << SYNCCTRL_RES);
	writew(tmp, pcdev->base + VIP_SYNCCTRL);
	...
	pcdev->cd_enb = readw(pcdev->base + VIP_CDENB);
...
static irqreturn_t vip_isr(int irq, void *data)
{
	...
	intc	= readw(pcdev->base + VIP_INTCTRL);
	odint	= readw(pcdev->base + VIP_ODINT);
...
static int __init lf2000_vip_probe(struct platform_device *pdev)
{
	...
	v4l2_ctrl_handler_init(&pcdev->ctrl_handler, lf2000_host_ops.num_controls);
	for (i = 0; i < ARRAY_SIZE(lf2000_camera_controls); i++) {
		printk(KERN_INFO "adding control %d: %d\n", i, lf2000_camera_controls[i].id);
